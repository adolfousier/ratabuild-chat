...
#[cfg(feature = "sqlite")]
pub async fn get_old_artifact_paths(pool: &SqlitePool, retention_days: u32) -> Result<Vec<String>, sqlx::Error> {
    let artifacts = sqlx::query_as::<_, (String,)>(
        "SELECT DISTINCT artifact_path FROM builds WHERE build_time < NOW() - INTERVAL '1 day' * $1"
    )
    .bind(retention_days as i32)
...
#[cfg(feature = "sqlite")]
pub async fn delete_old_builds_from_db(pool: &SqlitePool, retention_days: u32) -> Result<u64, sqlx::Error> {
    let result = sqlx::query(
        "DELETE FROM builds WHERE build_time < NOW() - INTERVAL '1 day' * $1"
    )
    .bind(retention_days as i32)
